...
| S2-6 | 刪除事件 | 事件消失 | ✅ |
| S2-7 | 切時鐘盤/日誌卡(操作前+操作後) | 三檢視一致渲染(display-based 切換) | ✅ |
...
- [x] 5 使用者情境:36/36 全部通過 ✅
- [x] 86 項回歸測試:全部綠燈 ✅
- [x] 10 項 E2E 標準:全部通過 ✅
- [x] 雙語截圖:en-US 長字串不溢出(en 文字平均 1.3–1.6 倍)
...
## 📊 6. 執行結果 · Execution Results(2026-08-25)
...
| 情境 | 結果 | 明細 |
|---|---|---|
| S1 首次使用 | **5/5 ✅** | Onboarding 三步+開始使用+重開不再出現 |
| S2 記錄一天 | **8/8 ✅** | 點空白新增→填表→儲存→編輯→重疊被拒→時鐘盤 SVG(操作前+操作後)→日誌卡→刪除 |
| S3 例行工事 | **3/3 ✅** | 勾選 streak+1→取消−1→統計卡計數 |
| S4 排程管理 | **3/3 ✅** | 排程畫面+空狀態+日誌卡今日排程 |
| S5 語言切換 | **5/5 ✅** | zh 預設→en 切換→持久→回切→雙語並列 |
...
| E2E-8 語言切換 | ✅ |
| E2E-9 console 零錯誤 | ✅ |
| E2E-10 截圖(.shots/acc-*.png) | ✅ |

### 截圖清單

| 檔案 | 內容 |
|---|---|
| `.shots/acc-s1-1-onboarding.png` | S1 Onboarding 步驟 1 |
| `.shots/acc-s1-3-today.png` | S1 進入主畫面 |
| `.shots/acc-s2-2-created.png` | S2 事件新增後 |
| `.shots/acc-s2-4-edited.png` | S2 事件編輯後 |
| `.shots/acc-s2-7-blocks.png` | S2 日誌卡檢視 |
| `.shots/acc-s3-routine.png` | S3 例行工事 |
| `.shots/acc-s4-schedule.png` | S4 排程管理 |
| `.shots/acc-s5-2-en.png` | S5 切換 en-US |
| `.shots/acc-s5-4-zh.png` | S5 切回 zh-TW |
